...
export function AccountProvider({ children }: { children: React.ReactNode }) {
  const [profiles, setProfiles] = useState<Account[]>(accountsData as Account[]);
  const [selectedProfile, setSelectedProfile] = useState<Account | null>(profiles[0] ?? null);
  ...
  return (
    <AccountContext.Provider value={{ profiles, selectedProfile, selectProfile, reloadProfiles }}>
      {children}
