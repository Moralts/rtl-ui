    ...
    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[minmax(0,1fr)] h-full items-stretch min-h-0">
      {INSTANCE_CARDS.map((card) => (
        <Link key={card.id} href={card.href} className="block h-full">
          <Card className="group relative overflow-hidden hover:shadow-xl transition-all duration-300 cursor-pointer h-full flex flex-col border hover:border-primary/50">
            {/* 悬停时的渐变背景 */}
            <div 
              className="absolute inset-0 opacity-0 group-hover:opacity-15 transition-opacity duration-300"
              style={{ background: getGradientStyle(card.colorFrom, card.colorTo) }}
            />
            {/* 卡片边框发光效果 */}
            ...
            <CardHeader className="relative">
              ...
              <div className={cn(
                ...
                <svg 
                  ...
                  className={cn("h-7 w-7 transition-colors duration-300", card.iconColor)} 
                  viewBox="0 0 24 24" 
                  fill="none" 
                  stroke="currentColor" 
                  strokeWidth="2"
              ...
              <CardDescription className="text-xs group-hover:text-muted-foreground transition-colors duration-300">{card.description}</CardDescription>
            </CardHeader>
            <CardContent className="relative flex-1 flex flex-col justify-between">
              <div className="space-y-2">
                {card.stats.map((stat, index) => (
                  <p key={index} className="text-xs text-muted-foreground group-hover:text-foreground/70 transition-colors duration-300">
                    {stat}
                  </p>
                ))}
              </div>
